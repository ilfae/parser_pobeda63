...
url_label = tk.Label(url_frame, text="Введите сайт:", font=("Helvetica", 12), bg="#1E1E1E", fg="white")
url_label.pack(side=tk.LEFT)

url_entry = tk.Entry(url_frame, width=22, bg="#424242", fg="#e1e3e6")
url_entry.configure(font=("Helvetica", 12))
url_entry.pack(side=tk.LEFT, padx=10)

paste_button = tk.Button(url_frame, compound="top", text="Вставить", command=lambda: paste_from_clipboard(), bg="white", fg="#1E293B", width=10)
paste_button.pack(side=tk.LEFT, padx=(0, 10))

num_pages_frame = tk.Frame(root, bg="#1E1E1E")
num_pages_frame.pack(padx=40, pady=20, fill=tk.BOTH)

num_pages_label = tk.Label(num_pages_frame, text="Укажите кол-во страниц на сайте:", font=("Helvetica", 12), bg="#1E1E1E", fg="white")
num_pages_label.pack(side=tk.LEFT, padx=(0, 10))

num_pages_entry = tk.Entry(num_pages_frame, width=5, bg="#424242", fg="#e1e3e6")
num_pages_entry.configure(font=("Helvetica", 12))
num_pages_entry.pack(side=tk.LEFT, padx=10)

run_button = tk.Button(num_pages_frame, compound="top", text="Запустить", command=lambda: threading.Thread(target=start_parser).start(), bg="#7E81FF", fg="white", width=10)
...
def show_ready_label():
    ...
    ready_label = tk.Label(button_frame, text="Готово ", font=("Helvetica", 12), bg="#1E1E1E", fg="#7E81FF")
